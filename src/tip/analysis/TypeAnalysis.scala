 ...
 * `implicit val analysisdeclData: TypeData = new DeclarationAnalysis(programNode).analyze()`.
...
class TypeAnalysis(program: AProgram)(implicit declData: DeclarationData) extends DepthFirstAstVisitor[Unit] with Analysis[TypeData] {
  ...
  def visit(node: AstNode, arg: Unit): Unit = {
    ...
    node match {
      ...
      case _: ANumber => unify(_, IntType()) // DONE
      case _: AInput => unify(_, IntType()) // DONE
      case is: AIfStmt => unify(is.guard, IntType()) // DONE
      case os: AOutputStmt => unify(os.exp, IntType()) // DONE
      case ws: AWhileStmt => unify(ws.guard, IntType()) // DONE
      case as: AAssignStmt =>
        as.left match {
          case id: AIdentifier => unify(id, as.right) // DONE
          case dw: ADerefWrite => unify(dw.exp, PointerType(as.right)) //DONE
          case dfw: ADirectFieldWrite =>
      ...
      case bin: ABinaryOp =>
        bin.operator match {
          ...
          // TW: choose 2 from 4 = 4 * 3 / 2 = 6
        }
      case un: AUnaryOp =>
        un.operator match {
          case DerefOp => unify(un.subexp, PointerType(un)) // DONE
        }
      case alloc: AAlloc => unify(alloc, PointerType(alloc.exp)) // DONE
      case ref: AVarRef => unify(ref, PointerType(ref.id)) // DONE
      case _: ANull => unify(_, PointerType(FreshVarType())) // DONE
      case fun: AFunDeclaration => unify(fun, FunctionType(fun.params, fun.stmts.ret.exp)) // DONE: TODO
      case call: ACallFuncExpr => unify(call.targetFun, FunctionType(call.args, call)) // DONE
      case _: AReturnStmt =>
      case rec: ARecord =>
